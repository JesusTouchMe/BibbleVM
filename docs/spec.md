...
|     LOAD_ST      |     0x8C     |      u16 index       |    Push 1     |               -               | Load a stack value located at the unsigned immediate `index` and pushes it to the stack.                                                                                              | -                         | 8C XX XX                   |
|      STORE       |     0x8D     |      u16 index       |       -       |               -               | Store the value in ACC into the stack slot located at the unsigned immediate `index`.                                                                                                 | -                         | 8D XX XX                   |
|     STORE_ST     |     0x8E     |      u16 index       |     Pop 1     |               -               | Pops a value from the stack and moves it into the stack slot located at the unsigned immediate `index`.                                                                               | -                         | 8E XX XX                   |
...
|       CALL       |     0x9A     | u32 target, u16 argc |  Pop `argc`   | ACC ← return value or garbage | Invokes a data section `CallEntry` referenced by `target`.                                                                                                                            | -                         | 9A XX XX XX XX XX XX       |

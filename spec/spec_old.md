...
|       LOAD       |     0x8B     |      u16 index       |       -        |      ACC ← S\[`index`\]       | Load a stack value located at the unsigned immediate `index` into ACC.                                                                                                                                            | -                         | 8B XX XX                   |
|     LOAD_ST      |     0x8C     |      u16 index       |     Push 1     |               -               | Load a stack value located at the unsigned immediate `index` and push it to the stack.                                                                                                                            | -                         | 8C XX XX                   |
|      STORE       |     0x8D     |      u16 index       |       -        |               -               | Store the value in ACC into the stack slot located at the unsigned immediate `index`.                                                                                                                             | -                         | 8D XX XX                   |
|     STORE_ST     |     0x8E     |      u16 index       |     Pop 1      |               -               | Pop a value from the stack and move it into the stack slot located at the unsigned immediate `index`.                                                                                                             | -                         | 8E XX XX                   |
...
|       JNZ        |     0x99     |      i16 branch      |       -        |               -               | Increments PC by `branch` if ACC is equal to true, tested as NOT equal to false (0).                                                                                                                              | -                         | 99 XX XX                   |
|       CALL       |     0x9A     | u32 target, u8 argc  |   Pop `argc`   | ACC ← return value or garbage | Invokes a data section `CallEntry` referenced by `target`. Pops `argc` values from the current stack frame and puts them on a new stack frame for the call entry.                                                 | -                         | 9A XX XX XX XX XX          |
|     CALL_EX      |     0x9B     | u32 target, u16 argc |   Pop `argc`   | ACC ← return value or garbage | Does the exact same as `CALL`, just with an extended `argc` operand.                                                                                                                                              | -                         | 9B XX XX XX XX XX XX       |
|     CALL_ST      |     0x9C     |       u16 argc       | Pop 1 + `argc` | ACC ← return value or garbage | Pops `argc` values from the stack for arguments, then pops 1 more value for the `CallEntry` address as a u32. Meant for use with instructions to resolve dynamic functions at runtime, not as a standalone thing. | -                         | 9C XX XX                   |
|    CALL_TINY     |     0x9D     | u16 target, u8 argc  |   Pop `argc`   | ACC ← return value or garbage | Does the exact same as `CALL`, just with a tiny `target` operand.                                                                                                                                                 | -                         | 9D XX XX XX                |
|   CALL_TINY_EX   |     0x9E     | u16 target, u16 argc |   Pop `argc`   | ACC ← return value or garbage | Does the exact same as `CALL_TINY`, just with an extended `argc` operand.                                                                                                                                         | -                         | 9E XX XX XX XX             |
|       RET        |     0x9F     |          -           |       -        |               -               | todo write description                                                                                                                                                                                            | -                         | 9F                         |